...
public class MemAwareSchedulingStrategy implements SchedulingStrategy {
	...
	private boolean isInMem(ScheduleTracker tracker, Stage readyStage) {
		//TODO: Throw error if tracker.getClusterDatasetRegistry().getMMP() is not of type MDFMemoryManagementPolicy
		if (readyStage == null ||
			readyStage.getInputDataReferences() == null || 
			readyStage.getInputDataReferences().size() == 0) {
			return true;
		}
		MDFMemoryManagementPolicy mmp = (MDFMemoryManagementPolicy)tracker.getClusterDatasetRegistry().getMMP();
		for (Set <DataReference> inputs : readyStage.getInputDataReferences().values()) {
			for (DataReference input : inputs) {
				if ( !mmp.datasetIsAllInMem(input) ) {
					return false;
				}
			}
		}
	...
	private Stage nextStageToSchedule(Stage head, ScheduleTracker tracker) {
		Stage toReturn = null;
		Stage holder = null;
		boolean inMemHold = false;
		boolean inMemRet = false;
